...
function AppShell() {
  const { open, initialCard, openAdd, closeAdd } = useAddCard();
  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#0a0e0a' }}>
      ...
      {/* Single global AddCardSheet — never unmounts, never re-animates on parent re-renders */}
      <AddCardSheet open={open} initialCard={initialCard} onClose={closeAdd} />
    </div>
  );
}
