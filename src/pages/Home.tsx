...
export default function Home() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0a0e0a' }}>
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8">
          <div className="flex items-center gap-2 py-3">
            ...
            <button
              ...
              className="p-2 rounded-xl transition-colors press-scale flex-shrink-0"
              style={{
              ...
              <div className="flex gap-2 overflow-x-auto pb-0.5">
                {LANGUAGE_FILTERS.map(l => (
                  <button key={l} onClick={() => { setLanguage(l); setPage(1); }}
                    ...
                    style={{ background: language === l ? '#00cc44' : '#111811', color: language === l ? '#000' : '#8fa88f', border: '1px solid ' + (language === l ? '#00cc44' : '#1e2e1e') }}>
                    {l}
                  </button>
                ))}
                ...
                {SORT_OPTIONS.map(s => (
                  <button key={s.value} onClick={() => { setSort(s.value); setPage(1); }}
                    className="text-[12px] font-semibold px-3 py-1.5 rounded-full whitespace-nowrap flex-shrink-0 press-scale"
                    style={{ background: sort === s.value ? '#1e2e1e' : 'transparent', color: sort === s.value ? '#e8f5e8' : '#4a5e4a', border: '1px solid ' + (sort === s.value ? '#2a3d2a' : 'transparent') }}>
                    {s.label}
                  </button>
                ))}
              </div>
              {setsData && (
                <div className="flex gap-2 overflow-x-auto pb-0.5">
                  <button onClick={() => setSetFilter('')}
                    className="text-[12px] font-semibold px-3 py-1.5 rounded-full whitespace-nowrap flex-shrink-0 press-scale"
                    style={{ background: !setFilter ? '#00cc44' : '#111811', color: !setFilter ? '#000' : '#8fa88f', border: '1px solid ' + (!setFilter ? '#00cc44' : '#1e2e1e') }}>
                    All Sets
                  </button>
                  {setsData.slice(0, 30).map(s => (
                    <button key={s.id} onClick={() => { setSetFilter(s.id); setPage(1); }}
                      ...
                      style={{ background: setFilter === s.id ? '#00cc44' : '#111811', color: setFilter === s.id ? '#000' : '#8fa88f', border: '1px solid ' + (setFilter === s.id ? '#00cc44' : '#1e2e1e') }}>
      ...
      <main className={cn('max-w-[1600px] mx-auto px-4 sm:px-8 pb-36', showFilters ? 'pt-44' : 'pt-20')}>
        ...
        {!query && trending.length > 0 && (
          <section className="mb-6">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <h2 className="text-[14px] font-bold text-[#e8f5e8] tracking-tight">Trending Today</h2>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-1">
              {trending.slice(0, 12).map(t => (
                <Link key={t.card.id} to={`/card/${t.card.id}`}
                  ...
                  style={{ background: '#111811', border: '1px solid #1e2e1e' }}>
                  {t.card.imageSmall && (
                    <img src={t.card.imageSmall} alt={t.card.name} className="w-full aspect-[2.5/3.5] object-contain p-1" style={{ background: '#0a0e0a' }} />
                  )}
                  <div className="px-2 pb-2 pt-1">
                    <p className="text-[10px] font-semibold truncate text-[#e8f5e8]">{t.card.name}</p>
        ...
        {!query && setsData && (
          <section className="mb-6">
            <div className="flex items-center gap-1.5 mb-3">
              <Package size={14} className="text-[#00cc44]" />
              <h2 className="text-[14px] font-bold text-[#e8f5e8] tracking-tight">Sets</h2>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-1">
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[14px] font-bold text-[#e8f5e8]">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8 gap-2.5">
          {isFetching && cards.length === 0
            ? Array.from({ length: 32 }).map((_, i) => <CardTileSkeleton key={i} />)
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-3 mt-6">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              className="px-4 py-2 rounded-xl text-[13px] font-semibold press-scale disabled:opacity-30"
